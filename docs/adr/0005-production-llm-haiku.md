...
model (`claude-3-5-sonnet-20241022`) was **retired** and would 404. A model
...
production responder. `LLM_MODEL` is env-configurable, so the model is a knob you
...
- Haiku is the weakest current Claude tier, so it's likely lower accuracy on the
  ≥95% goal than Sonnet. Accepted because the model is a config swap and the eval
  will quantify the trade-off (don't guess, measure).
