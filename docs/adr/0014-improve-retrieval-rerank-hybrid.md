...
quality (generation can only use what it's handed), the ~40%
of questions whose answer page isn't in the top-5 get weak or muddled answers.
Concretely: "how fast does SCT Inst settle?" missed the crisp "5 seconds" page
(p26) because it never ranked into the top-5. We want to raise recall without
regressing cost/latency unacceptably.
...
Implication: try **hybrid search (vector + BM25)** first, since it can pull the
missing exact-term pages into the candidate set; reranking is a secondary,
lower-headroom lever. (Small sample, so treat it as directional; grow the golden
set for stable numbers.)
