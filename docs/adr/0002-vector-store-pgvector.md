...
- **Dedicated vector DBs** (Chroma, Qdrant, Pinecone). More specialised ANN
  features, but a new technology to learn/operate, and Pinecone adds a hosted
  dependency + cost.
...
- Two index choices exist: **HNSW** (chosen — no training, incremental inserts,
  best recall/speed, more memory) vs IVFFlat (needs a representative set to
  train, cheaper memory). HNSW suits a small, growing corpus.
