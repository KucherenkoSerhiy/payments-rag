...
## Context
`payments_rag/` was a flat package of ~9 modules, and the folder gave no signal of
...
2. **Extract `adapters/llm.py`** (Anthropic client + `complete_json`) from the
   ...
   pure flow: retrieve, build prompt, call the LLM, then map citations.
3. **Adapters stay modules** (functions + a lazy client singleton), not classes.

## Alternatives
- **Package by technical layer** (`controllers/ services/ models/`). Rejected: it
  screams *framework* instead of *domain*.
