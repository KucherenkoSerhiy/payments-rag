...
What changed is the goal, not the product: Track B (skill) wanted the reranking
*mechanism* hands-on — bi-encoder vs cross-encoder, fanout → rescore → top-k. So
we built it to learn it and to measure it against the golden set, explicitly not
as a product default.

## Decision
1. **Instantiate the cross-encoder as an LLM call, not a local model.** The
...
value-limits) are never fetched even at 20 — recall-bound, untouchable by
reranking. Confirms ADR-0014's diagnosis: **the bottleneck is recall, not ranking.**
...
on real wall-clock rather than read in a doc.
