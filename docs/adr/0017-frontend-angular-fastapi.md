...
2. **Cloud deploy (M8) needs an API layer anyway.** A FastAPI backend is not extra
...
- **Another Python UI** (Reflex / NiceGUI / Gradio). Rejected: still not the
  control we want, and doesn't play to the owner's Angular strength.
...
## Migration path
1. **FastAPI backend** first — endpoints for `ask`, `evals` (retrieval + answer,
